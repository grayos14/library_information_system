...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#pengembalianCard').on('click', '#addBtn', function(e) {
            e.preventDefault();
            const urlParams = "<?= base_url('ajax/create/pengembalian') ?>";
            requestAjax(urlParams, 'GET', 'html', function(response) {
                $('#formModal .modal-title').html('Form Pengajuan Pengembalian');
        ...
        $('#formModal').on('click', '#saveButton', function(e) {
            e.preventDefault();
            const urlParams = "<?= base_url('create/pengembalian') ?>";
            const formData = $('#pengembalianForm').serialize();
        ...
        $('#formModal').on('click', '#updateButton', function(e) {
            e.preventDefault();
            const urlParams = "<?= base_url('update/pengembalian') ?>";
            const formData = $('#editPengembalianForm').serialize();
            requestAjax(urlParams, 'POST', 'json', function(response) {
                if(response.code == 200) {
                    alert(response.message);
                    $('#formModal').modal('hide');
